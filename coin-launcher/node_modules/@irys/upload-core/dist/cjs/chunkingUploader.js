...
const s2ai_1 = tslib_1.__importDefault(require("./s2ai"));
    ...
    async runUpload(dataStream, transactionOpts) {
        ...
        if (!isTransaction) {
            tx = this.bundles.createData('', this.tokenConfig.getSigner(), {
                ...transactionOpts,
                anchor: transactionOpts?.anchor ??
                    crypto_1.default.randomBytes(32).toString('base64').slice(0, 32),
            });
            const raw = tx.getRaw();
        ...
        if (transactionOpts?.upload?.paidBy)
            finalHeaders[types_1.UploadHeaders.PAID_BY] = transactionOpts.upload.paidBy;
        ...
        finishUpload.data.verify = utils_1.default.verifyReceipt.bind({}, this.bundles, finishUpload.data.data);
