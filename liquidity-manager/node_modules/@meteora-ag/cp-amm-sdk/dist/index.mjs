...
var LayoutVersion = /* @__PURE__ */ ((LayoutVersion2) => {
  LayoutVersion2[LayoutVersion2["V0"] = 0] = "V0";
  LayoutVersion2[LayoutVersion2["V1"] = 1] = "V1";
  return LayoutVersion2;
})(LayoutVersion || {});
...
var CURRENT_POOL_VERSION = 1;
var DEAD_LIQUIDITY = new BN(100).shln(SCALE_OFFSET);
var NUM_REWARDS = 2;
...
function getTotalLockedLiquidity(vestingData) {
  return vestingData.innerVesting.cliffUnlockLiquidity.add(
    vestingData.innerVesting.liquidityPerPeriod.mul(
      new BN4(vestingData.innerVesting.numberOfPeriod)
    )
  );
}
...
// src/math/poolFees/baseFee/baseFeesHandler.ts
import BN11 from "bn.js";

// src/math/poolFees/baseFee/rateLimiter.ts
...
function splitFees(poolFees, feeAmount, hasReferral) {
  let protocolFee = mulDiv(
    feeAmount,
    new BN6(poolFees.protocolFeePercent),
    new BN6(100),
    1 /* Down */
  );
  let tradingFee = feeAmount.sub(protocolFee);
  const compoundingFeeBps = new BN6(poolFees.compoundingFeeBps);
  let compoundingFee;
  let claimingFee;
  if (compoundingFeeBps.gt(new BN6(0))) {
  ...
  if (hasReferral) {
    referralFee = mulDiv(
      protocolFee,
      new BN6(poolFees.referralFeePercent),
      new BN6(100),
      1 /* Down */
    );
  } else {
    referralFee = new BN6(0);
  }
...
function getMaxFeeBps(feeVersion) {
  switch (feeVersion) {
    case 0:
...
function getRateLimiterMaxBaseFeeNumerator(includedFeeAmount, referenceAmount, cliffFeeNumerator, maxFeeBps, feeIncrementBps) {
  return getFeeNumeratorFromIncludedFeeAmount(
    includedFeeAmount,
    referenceAmount,
    cliffFeeNumerator,
...
// src/math/poolFees/baseFee/feeTimeScheduler.ts
import BN9 from "bn.js";

// src/math/poolFees/baseFee/feeScheduler.ts
...
// src/math/poolFees/baseFee/feeMarketCapScheduler.ts
...
var FeeRateLimiter = class {
  ...
  validate(collectFeeMode, activationType, feeVersion) {
...
var FeeMarketCapScheduler = class {
  ...
  validate(_collectFeeMode, _activationType, feeVersion) {
...
function getBaseFeeHandlerFromBorshData(rawData) {
  const data = Buffer.from(rawData);
...
// src/math/liquidity/compoundingLiquidity.ts
import BN13 from "bn.js";
function getInitialCompoundingPoolInformation(sqrtPrice, liquidity) {
  if (liquidity.lte(DEAD_LIQUIDITY)) {
    throw new InvalidMinimumLiquidityError();
  }
  const tokenAAmount = getPoolCreationAmountAFromLiquidityDeltaForCompoundingLiquidity(
    sqrtPrice,
    liquidity
  );
  const tokenBAmount = getPoolCreationAmountBFromLiquidityDeltaForCompoundingLiquidity(
    sqrtPrice,
    liquidity
  );
  return {
...
function calculateAtoBFromAmountInForCompoundingLiquidity(tokenAAmount, tokenBAmount, amountIn) {
  ...
  return {
    amountLeft: new BN13(0),
    outputAmount,
    nextSqrtPrice: new BN13(0)
  };
}
...
function getPoolCreationAmountBFromLiquidityDeltaForCompoundingLiquidity(sqrtPrice, liquidity) {
  ...
  const denominator = new BN13(1).ushln(128);
...
// src/math/liquidity/concentratedLiquidity.ts
import BN14 from "bn.js";
function getInitialConcentratedLiquidityPoolInformation(sqrtMinPrice, sqrtMaxPrice, sqrtPrice, liquidity) {
...
function getAmountBFromLiquidityDeltaForConcentratedLiquidity(lowerSqrtPrice, upperSqrtPrice, liquidity, rounding) {
  ...
  if (rounding === 0 /* Up */) {
    const denominator = new BN14(1).ushln(shift);
...
// src/math/liquidity/liquidityHandler.ts
...
function getInitialPoolInformation(collectFeeMode, sqrtMinPrice, sqrtMaxPrice, sqrtPrice, liquidity) {
  if (collectFeeMode === 2 /* Compounding */) {
...
function getSwapResultFromExactInput(poolState, amountIn, feeMode, tradeDirection, currentPoint) {
  let actualProtocolFee = new BN16(0);
  let actualClaimingFee = new BN16(0);
  let actualCompoundingFee = new BN16(0);
  let actualReferralFee = new BN16(0);
...
// src/helpers/validation.ts
function validateFeeTimeScheduler(numberOfPeriod, periodFrequency, reductionFactor, cliffFeeNumerator, baseFeeMode, feeVersion) {
  if (!periodFrequency.eq(new BN17(0)) || numberOfPeriod !== 0 || !reductionFactor.eq(new BN17(0))) {
    if (numberOfPeriod === 0 || periodFrequency.eq(new BN17(0)) || reductionFactor.eq(new BN17(0))) {
  ...
  validateFeeFraction(minFeeNumerator, new BN17(FEE_DENOMINATOR));
  validateFeeFraction(maxFeeNumerator, new BN17(FEE_DENOMINATOR));
  if (minFeeNumerator.lt(new BN17(MIN_FEE_NUMERATOR)) || maxFeeNumerator.gt(getMaxFeeNumerator(feeVersion))) {
...
function validateFeeMarketCapScheduler(cliffFeeNumerator, numberOfPeriod, sqrtPriceStepBps, reductionFactor, schedulerExpirationDuration, feeMarketCapSchedulerMode, feeVersion) {
  if (reductionFactor.lte(new BN17(0))) {
    throw new InvalidFeeMarketCapSchedulerError();
  }
  if (sqrtPriceStepBps.lte(new BN17(0))) {
    throw new InvalidFeeMarketCapSchedulerError();
  }
  if (schedulerExpirationDuration.lte(new BN17(0))) {
    throw new InvalidFeeMarketCapSchedulerError();
  }
  if (numberOfPeriod <= 0) {
    throw new InvalidFeeMarketCapSchedulerError();
  ...
  if (minFeeNumerator.lt(new BN17(MIN_FEE_NUMERATOR)) || maxFeeNumerator.gt(maxAllowedFeeNumerator)) {
    throw new ExceedMaxFeeBpsError();
  }
  return true;
}
...
function validateFeeRateLimiter(cliffFeeNumerator, feeIncrementBps, maxFeeBps, maxLimiterDuration, referenceAmount, collectFeeMode, activationType, feeVersion) {
  ...
  const maxFeeNumeratorFromBps = toNumerator(
    new BN17(maxFeeBps),
    new BN17(FEE_DENOMINATOR)
  );
  if (cliffFeeNumerator.lt(new BN17(MIN_FEE_NUMERATOR)) || cliffFeeNumerator.gt(maxFeeNumeratorFromBps)) {
    throw new InvalidFeeRateLimiterError(
  ...
  const maxLimiterDurationLimit = activationType === 0 /* Slot */ ? new BN17(MAX_RATE_LIMITER_DURATION_IN_SLOTS) : new BN17(MAX_RATE_LIMITER_DURATION_IN_SECONDS);
  if (new BN17(maxLimiterDuration).gt(maxLimiterDurationLimit)) {
    throw new InvalidFeeRateLimiterError(
      "maxLimiterDuration exceeds limit for activation type"
    );
  }
  const feeIncrementNumerator = toNumerator(
    new BN17(feeIncrementBps),
    new BN17(FEE_DENOMINATOR)
  );
  if (feeIncrementNumerator.gte(new BN17(FEE_DENOMINATOR))) {
    throw new InvalidFeeRateLimiterError(
  ...
  if (minFeeNumerator.lt(new BN17(MIN_FEE_NUMERATOR)) || maxFeeNumeratorFromAmount.gt(getMaxFeeNumerator(feeVersion))) {
...
function validateActivationType(activationType) {
  const allowed = [1 /* Timestamp */, 0 /* Slot */];
  if (!allowed.includes(activationType)) {
    throw new InvalidActivationTypeError();
  }
}
function validatePriceRange(collectFeeMode, sqrtMinPrice, sqrtMaxPrice) {
  if (collectFeeMode === 2 /* Compounding */) {
    return;
  }
...
function validateInitialSqrtPrice(collectFeeMode, initSqrtPrice, sqrtMinPrice, sqrtMaxPrice) {
  if (collectFeeMode === 2 /* Compounding */) {
...
function validateCompoundingFee(collectFeeMode, compoundingFeeBps) {
  if (collectFeeMode === 2 /* Compounding */) {
...
function validatePoolFees(poolFees, collectFeeMode, activationType, feeVersion = CURRENT_POOL_VERSION) {
...
function validateCustomizablePoolParams(params) {
  const {
    collectFeeMode,
    activationType,
    sqrtMinPrice,
  ...
  if (tokenAAmount.lte(new BN17(0)) && tokenBAmount.lte(new BN17(0))) {
...
function validateLockPositionParams(params) {
  ...
  if (numberOfPeriod > 0) {
    if (periodFrequency.lte(new BN17(0)) || liquidityPerPeriod.lte(new BN17(0))) {
  ...
  if (totalLockAmount.lte(new BN17(0))) {
    throw new InvalidVestingInfoError(
...
function validateRewardDuration(rewardDuration) {
  if (rewardDuration.lt(new BN17(MIN_REWARD_DURATION)) || rewardDuration.gt(new BN17(MAX_REWARD_DURATION))) {
...
function encodeFeeTimeSchedulerParams(cliffFeeNumerator, numberOfPeriod, periodFrequency, reductionFactor, baseFeeMode) {
  const feeTimeScheduler = {
    cliff_fee_numerator: new BN18(cliffFeeNumerator.toString()),
    number_of_period: numberOfPeriod,
    period_frequency: new BN18(periodFrequency.toString()),
    reduction_factor: new BN18(reductionFactor.toString()),
...
function encodeFeeRateLimiterParams(cliffFeeNumerator, feeIncrementBps, maxLimiterDuration, maxFeeBps, referenceAmount) {
  const feeRateLimiter = {
    ...
    reference_amount: new BN18(referenceAmount.toString()),
...
function parseFeeSchedulerSecondFactor(secondFactor) {
  return new BN19(Buffer.from(secondFactor), "le");
}
...
function bpsToFeeNumerator(bps) {
  return new BN19(bps * FEE_DENOMINATOR).divn(BASIS_POINT_MAX);
}
function feeNumeratorToBps(feeNumerator) {
  return feeNumerator.muln(BASIS_POINT_MAX).div(new BN19(FEE_DENOMINATOR)).toNumber();
}
...
function fromDecimalToBN(value) {
  return new BN19(value.floor().toFixed());
}
function getFeeTimeSchedulerParams(startingBaseFeeBps, endingBaseFeeBps, baseFeeMode, numberOfPeriod, totalDuration) {
  ...
  const periodFrequency = new BN19(totalDuration / numberOfPeriod);
  let reductionFactor;
  if (baseFeeMode == 0 /* FeeTimeSchedulerLinear */) {
    ...
    reductionFactor = new BN19(BASIS_POINT_MAX * (1 - decayBase));
  }
...
function getRateLimiterParams(baseFeeBps, feeIncrementBps, referenceAmount, maxLimiterDuration, maxFeeBps, tokenBDecimal, activationType) {
  ...
  if (feeIncrementNumerator.gte(new BN19(FEE_DENOMINATOR))) {
    throw new Error(
  ...
  const deltaNumerator = new BN19(poolMaxFeeNumerator).sub(cliffFeeNumerator);
  const maxIndex = deltaNumerator.div(feeIncrementNumerator);
  if (maxIndex.lt(new BN19(1))) {
    throw new Error("Fee increment is too large for the given base fee");
  }
  if (cliffFeeNumerator.lt(new BN19(MIN_FEE_NUMERATOR)) || cliffFeeNumerator.gt(new BN19(poolMaxFeeNumerator))) {
...
function getDynamicFeeParams(baseFeeBps, maxPriceChangeBps = MAX_PRICE_CHANGE_BPS_DEFAULT) {
  ...
  const sqrtPriceRatioQ64 = new BN19(
  ...
  const maxVolatilityAccumulator = new BN19(deltaBinId.muln(BASIS_POINT_MAX));
  const squareVfaBin = maxVolatilityAccumulator.mul(new BN19(BIN_STEP_BPS_DEFAULT)).pow(new BN19(2));
  const baseFeeNumerator = new BN19(bpsToFeeNumerator(baseFeeBps));
  const maxDynamicFeeNumerator = baseFeeNumerator.muln(20).divn(100);
  const vFee = maxDynamicFeeNumerator.mul(new BN19(1e11)).sub(new BN19(99999999999));
...
var CpAmm = class {
  ...
  getStaticConfigs() {
    return __async(this, null, function* () {
      ...
      return configAccounts.filter(
        (config) => config.account.configType === 0 && config.account.vaultConfigKey.equals(PublicKey6.default) && config.account.poolCreatorAuthority.equals(PublicKey6.default)
      );
    });
  }
  /**
  ...
  getQuote(params) {
    ...
    const currentPoint = activationType ? new BN20(currentTime) : new BN20(currentSlot);
  ...
  preparePoolCreationParams(params) {
    ...
    if (tokenAAmount.eq(new BN20(0)) && tokenBAmount.eq(new BN20(0))) {
  ...
  createCustomPool(params) {
    return __async(this, null, function* () {
      ...
      const {
        ...
        tokenBMint,
        tokenAAmount,
        tokenBAmount: tokenBMint.equals(NATIVE_MINT2) ? max(tokenBAmount, new BN20(1)) : tokenBAmount,
        payer,
        positionNft,
        tokenAProgram,
        tokenBProgram
  ...
  swap2(params) {
    return __async(this, null, function* () {
      ...
      if (inputTokenMint.equals(NATIVE_MINT2)) {
        const amount2 = swapMode === 0 /* ExactIn */ || swapMode === 1 /* PartialFill */ ? amount0 : amount1;
        const wrapSOLIx = wrapSOLInstruction(
          receiver != null ? receiver : payer,
          inputTokenAccount,
  ...
  lockPosition(params) {
    return __async(this, null, function* () {
      ...
      if (innerPosition) {
        return yield this._program.methods.lockInnerPosition({
          cliffPoint,
          periodFrequency,
          cliffUnlockLiquidity,
          liquidityPerPeriod,
